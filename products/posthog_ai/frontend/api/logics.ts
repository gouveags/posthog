...
// `runStreamLogic` publishes tool-call lifecycle events (resolved names) + a run-lifecycle event on
// terminal; subscribe to react when the agent invokes a specific tool. `useToolStreamListener`
// registers a subscription for a mount's life.
...
// MCP tool calls (foreground-gated, replay-excluded) so a scene can apply a generated query back.
