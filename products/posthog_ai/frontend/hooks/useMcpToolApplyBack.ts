...
/** When the matched apply-back reaction fires: once at run terminal, or immediately per completion. */
...
export interface UseMcpToolApplyBackOptions {
     ...
     * the foreground run reaches a terminal status. `'completed'`: apply immediately for each match.
     */
    applyOn?: ApplyOn
}
 ...
 * `'terminal'` mode the last matching completed event wins and `onApply` fires once when the run
...
export function useMcpToolApplyBack({ tools, onApply, applyOn = 'terminal' }: UseMcpToolApplyBackOptions): void {
    ...
    // Terminal mode: the last matching completed event, flushed on the run's terminal signal.
    ...
    useEffect(() => {
        ...
        registerToolListener(listenerId, {
            ...
            onRunTerminal: (_event: RunLifecycleEvent) => {
                ...
                // Clear before applying so the fire-once terminal signal can't double-apply and the next
                // run on this stream starts from an empty buffer.
                bufferedEventRef.current = null
                apply(buffered)
            },
