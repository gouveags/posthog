...
async def evaluate_cohort_batch_activity(input: EvaluateCohortBatchInput) -> EvaluateCohortBatchOutput:
    ...
    async def _run_one_cohort(manifest: CohortManifest) -> tuple[dict[str, int], list[NotifiedAlert]]:
        ...
        async with semaphore:
            ...
            try:
                saved, failed, conflicted = (await save_cohort_async(dispatched, now)) if dispatched else ([], [], [])
            except Exception as e:
            ...
            for _ in failed:
                ...
                local_stats["errored"] += 1
            # Conflicted: a control-plane write won; the evaluation was discarded,
            # not errored. Checked only.
