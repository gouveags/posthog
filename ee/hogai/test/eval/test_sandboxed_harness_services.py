...
from ee.hogai.eval.sandboxed.harness.services import package_local_skills_archive, start_mcp_server
...
def test_start_mcp_server_isolates_the_selected_skill_delivery(
    ...
    with (
        ...
        patch("ee.hogai.eval.sandboxed.harness.services.settings.BASE_DIR", tmp_path),
        patch(
            "ee.hogai.eval.sandboxed.harness.services.LONG_LIVED_SUBPROCESSES.start", return_value=(MagicMock(), stop)
        ) as start,
    ):
